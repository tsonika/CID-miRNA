...
enum TYPE_OF_RULE
{ _UNKNOWN_RULE = 0, _TERM_RULE = 1, _NONTERM_RULE = 2 };
...
//Friend Functions required for direct member access to these functions
    ...
    // as there should be no NT which does not give a production, i.e. only occurs on RHS
      ...
      public:SCFGrule ():RuleScore (MINUSINF), UsedOnceFlag (false),
        RuleNumber (0), No_of_RHSsymbols (0), RuleType (_UNKNOWN_RULE)
    {
    }
