...
enum TYPE_OF_LIST
{ _UNKNOWN = 0, _UPLIST = 1, _DOWNLIST = 2, _PARENTREF = 3, _TERMRULE = 4 };
...
//Friend Function required for direct member access to these functions
    ...
    friend void FillRestCellProcess1 (const vector < CykMatrixCell > &,
                                      CykMatrixCell &, int, int);
    friend bool FillRestCellProcess2 (const vector < CykMatrixCell > &,
                                      CykMatrixCell &, int, int);
    friend int main ();
    friend bool IsPossibleTree (const vector < CykMatrixCell > &);
    friend string BuildTrace (const vector < CykMatrixCell > &);
    friend void TraceFurther (ElementCoordinates,
                              const vector < CykMatrixCell > &,
                              ostringstream &);
...
//Friend Function required for direct member access to these functions
    ...
    ElementListItr DownListItr;
      ...
      public:CykMatrixCell ():UpListCount (0), DownListCount (0),
        UpListItr (UpList.begin ()), DownListItr (DownList.begin ())
    {
    }
...
class cartesian1
{
    ...
    friend void FillRestCellProcess1 (const vector < CykMatrixCell > &mat,
                                      CykMatrixCell & cykmc, int i, int j);
