 ...
 */
 ...
 char cachefileseq[2000001][MAXLEN+1];
 time_t t1, t2;
 fstream fout;
...
for(filectr = 0; filectr < MAXFILES; ++filectr, ++filecount[1])
{
...
strcpy(infile,"incyk");
strcat(infile,filecount);

gm[filectr].open(infile,ios::in);
if(!gm[filectr].good())
{
 cout << "\n Error Opening Sequence file"<<filectr+1<<" \n";
 gm[filectr].close();
 continue;
 }

for(pxx = 0; pxx <= 2000000; ++pxx)
 for(qxx = 0; qxx <= MAXLEN; ++qxx)
	 cachefileseq[pxx][qxx] = '\0';

for(pxx = 0; (pxx <= 2000000) && (!gm[filectr].eof()); ++pxx)
gm[filectr] >> cachefileseq[pxx];

gm[filectr].close();


// THE MAIN FOR LOOP WHERE EACH ITERATION IS INDEPENDENT and REPRESENTS A DIFFERENT SEQUENCE
...
seqlen = strlen(cachefileseq[pxx]);
...
for(i = 0; ((i < seqlen) && (!skipsequenceflag)); ++i)
 for(v = 0; ((v < NSYM) && (!skipsequenceflag)); ++v)
 {
  ...
  base = code_generator(cachefileseq[pxx][i]);
...
long cyk::code_generator(char x)
{
 if((x == 'a')||(x == 'A')) return 0;
 if((x == 'u')||(x == 'U')) return 1;
 if((x == 'g')||(x == 'G')) return 2;
...
// and second line says b[4][5] = 0.55
 ...
 strcpy(filename,"finalprobs.txt");
