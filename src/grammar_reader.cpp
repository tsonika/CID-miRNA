...
void SCFGrule::CountUpdate (void)
{
    ...
    while (iss >> temp) {
        ++count;
        if (!isupper (temp[0])) {       //If the rule does not begin with upper case
            if ((count != 1) || (!isalpha (temp[0]))) {
                ...
                if (T.empty ()) {
                    T = dummy;
                    ++Tcount;
                } else if ((T.find (dummy) < 0)
                           || (T.find (dummy) >= T.length ())) {
                    T += dummy;
                    ++Tcount;
                }
        ...
        else {                  //If the rule begins with upper case= it is a non-terminal rule
            dummy = star + temp + star;
...
bool SCFGrule::ValidateGrammar (void)
{
    ...
    for (int i = 0; i < T.length (); ++i) {
        if ((T[i] == '*') || (islower (T[i])))
            ...
            cout << endl << "Some Error in Terminal string : " << T.
                substr (i - 1, 3) << endl << T;
            return false;
        }
    }
...
vector < SCFGrule > ReadGrammarFile (ifstream * grammarfileptr)
{
    ...
    if (grammarfileptr == NULL) {
        ...
        if (grammarfileptr == NULL) {
            cout << endl <<
                "Unable to Read or Allocate memory for reading Grammar File"
                << endl;
            exit (1);
        }
        ...
        if (!grammarfileptr->good ()) {
            cout << endl << "Unable to Read Grammar File : " << filename <<
                endl;
            exit (1);
        }
    }
