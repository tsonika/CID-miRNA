...
void ALPHA::initialize_alpha(void)
{
	int i,j,v;
			...
			for(v = 0; v < NSYM; ++v)
			{
				for(k = i; k < j; ++k)
				{
					for(z = 0; z < NSYM; ++z)
					{
						for(y = 0; y < NSYM; ++y)
						{
							if(valida[v][y][z] && validalpha[i][k][y] && validalpha[k+1][j][z])
							{
								alpha[i][j][v] += (alpha[i][k][y] * alpha[k+1][j][z] * a[v][y][z]) ;
...
void BETA::initialize_beta(void)
{
	int i,j,v;
				...
				if(i > 0)
				{
					for(k = 0; k < i; ++k)
					{
						for(z = 0; z < NSYM; ++z)
						{
							for(y = 0; y < NSYM; ++y)
							{
								if(valida[y][z][v] && validalpha[k][i-1][z] && validbeta[k][j][y])
								{
									sum1 += ( alpha[k][i-1][z] * beta[k][j][y] * a[y][z][v] );
								}
							}
						}
					}
				...
				if(j < (seqlen-1))
				{
					for(k = j+1; k < seqlen; ++k)
					{
						for(z = 0; z < NSYM; ++z)
						{
							for(y = 0; y < NSYM; ++y)
							{
								if(valida[y][v][z] && validalpha[j+1][k][z] && validbeta[i][k][y])
								{
									sum2 += ( alpha[j+1][k][z] * beta[i][k][y] * a[y][v][z] );
								}
							}
						}
					}
...
void reestimation::updateB(void)
{
	long double sum1, sum2;
	int i, j, v, t;
	...
	for(v = 0; v < NSYM; ++v)
		for(t = 0; t < TSYM; ++t)
		{
			...
			for(i = 0; i < seqlen; ++i)
			{
				for(j = i; j < seqlen; ++j)
				{
					if(validbeta[i][j][v] && validalpha[i][j][v])
...
void reestimation::updateA(void)
{
	int i,j,k,v,y,z;
	long double sum1, sum2;
			...
			for(z = 0; z < NSYM; ++z)
			{
				...
				for(i = 0; i < (seqlen-1); ++i)
					for(j = i+1; j < seqlen; ++j)
					   ...
					   {
					    for(k = i; k < j; ++k)
						 if(validalpha[i][k][y] && validalpha[k+1][j][z])
							sum1 += ( beta[i][j][v] * a[v][y][z] * alpha[i][k][y] * alpha[k+1][j][z] );
					   }
...
class IOcontrol {
private :
	void initializer(void); // set Na Nb Da Db to zero
...
void IOcontrol::initializer(void)
{
	...
	while(!fvr.eof())
	{
		...
		if((ch == 'a') || (ch == 'A'))
		{
			...
			if((pm[0] < NSYM) &&(pm[1] < NSYM) &&(pm[2] < NSYM))
			{
				...
				cout << "\n a : "<< pm[0] <<"\t" << pm[1] << "\t"<< pm[2] << "\t"<< (a[pm[0]][pm[1]][pm[2]]);	
			}
		}
		else
		{ 
...
// for each sequence read,make modsequence, run alpha and beta calcs, and reestimate
	...
	for(i = 0; i < num_of_seq; ++i)
	{
		...
		cout << endl << " Analyzing Seq. " << "# " << i+1 <<"\t Length : "<<seqlen;
		...
		for(j = 0; j < seqlen; ++j)
		{
			...
			{
				cout << endl << "Seq # " << i+1 << " Illegal char found. Rejected";
		...
		{
			cout << endl << "Seq # " << i+1 << " sequence prob zero. Rejected";
			++reject;
			skipsequenceflag = true;
		}
...
void IOcontrol::finalcalculator(void)
{
	probreader();
	seqreader();
