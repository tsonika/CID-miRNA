...
public :
	...
	void initializer2(char filein[MAXPATH], char fileout[MAXPATH], unsigned long winlen, unsigned long minr, unsigned long maxr, unsigned long int cachelength = 0);
	void freememory(void);


};

void parser::cacheparser(void)
{
	...
	if(cykoutputflag == 'N')
	fout[filectr] <<"\n !!  SCANNING STEM LENGTH ["<< windowlen <<"] IN CACHE SIZE ["<< cachelen << "]  !!\n\n";
	...
	{       foundmatch = 0;
		...
		if(!complementor()) continue;

		filler(searchwindow,cachecur+windowlen+minrange, maxrange-minrange + windowlen);
		...
		while(ptr)
		{
			foundmatch = 1;
			// *ptr = '\0';
			seqlen = windowlen+minrange+strlen(searchwindow)-strlen(ptr)+windowlen;
			...
			{
			  if(cykoutputflag == 'N')
			  fout[filectr] << "\n> Position : " << cachecur << "\t Length : " << seqlen <<"\n";
		...
		// This particular portion is calculating the acceptance rate
			if((cachecur > curwindowstart) && (cachecur <= curwindowend) && ((cachecur + seqlen) > curwindowend))
	...
	for(modectr = 0; modectr <= (maxrange - minrange) ; ++modectr)
	{
	 if(maxmode < seqlentracker[modectr])
		{ maxmode = seqlentracker[modectr]; posmaxmode = modectr; }
	}
	...
	for(modectr = 0; modectr <= (maxrange - minrange); ++modectr)
	 stddevsum += seqlentracker[modectr] * (posmaxmode - modectr) * (posmaxmode - modectr);
	...
	cout << "\n\n End of Parsing !!"
	     ...
	     <<"\n Please see file ["<< outfile <<" AA to "<< filecount<<"] for output !!\n";
	 ...
	 for(modectr = 0; modectr <= (maxrange-minrange-3); modectr += 3)
	 {
	  ...
	  for(colctr = 0; colctr < 3; ++colctr)
	    cout <<"| "<<modectr + colctr + 2*windowlen + minrange <<"\t\t"<< seqlentracker[modectr+colctr]<<"\t";
	  cout<<"|";
	  }
	  cout<<"\n ";
...
void parser::filler(char* str, unsigned long start, unsigned long len)
{
...
int parser::complementor(void)
{
	unsigned long int ctr = 0;
	strcpy(compwindow,window);
	while((ctr < windowlen) && (window[ctr] != '\0'))
	{
		if(window[ctr] == 'A') compwindow[windowlen-1-ctr] = 'U';
		else if(window[ctr] == 'U') compwindow[windowlen-1-ctr] = 'A';
		else if(window[ctr] == 'G') compwindow[windowlen-1-ctr] = 'C';
		else if(window[ctr] == 'C') compwindow[windowlen-1-ctr] = 'G';
		else
		{
...
void parser::initializer2(char filein[MAXPATH], char fileout[MAXPATH], unsigned long int winlen, unsigned long int minr, unsigned long int maxr, unsigned long int cachelength)
{
...
void parser::mem_allocator(void)
{
	filelen = 0;
	char ch;
	...
	if(!cache || !window || !compwindow || !searchwindow || !sequence || !seqlentracker)
	{
...
char parser::altcomplement(char ichar)
{
 if (ichar == 'A') return 'G';
 else if(ichar == 'C') return 'U';
 return ichar;
}
 
char* parser::mystrstr(char* targetstr, char* searchstr)
{
...
char parser::DNAtoRNAcomplement(char ichar)
{
 if(ichar == 'A') return 'U';
 else if(ichar == 'T') return 'A';
 else if(ichar == 'G') return 'C';
 else if(ichar == 'C') return 'G';
 else return ichar;
 }
 ...
 for(i = 0; i < len; ++i)
   if((checkstr[i] != 'A') && (checkstr[i] != 'U') && (checkstr[i] != 'G') && (checkstr[i] != 'C'))
   {
   seqcount--;
   fileseqcounter--;
   return false;
