...
for (itr1=0; itr1< matcel.UpList.size(); itr1++)//Run through the Upper List of the cell
    ...
    for (int x=0;x < RuleList[0].RuleCount;x++)//Run through each rule
        {
        if (RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS == tmpNT)//If the non-terminal in the upperlist is directly given by a terminal in the RuleList
           {
           ...
           if(RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS.find(tmpNT) == 0)//The non-terminal in the upperlist is NOT directly given by a terminal in the RuleList, but is the first non-terminal on RHS
           {
           CykListElement tempcle;
...
for (itr1=0; itr1< cykmc.UpList.size(); itr1++)
    ...
    for (int x=0;x < RuleList[0].RuleCount;x++)//Run through each rule
        {
        if (RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS == dummy)//If the non-terminal we are looking for is directly given by a terminal in the RuleList
           {
           CykListElement tempcle;
           tempcle.ruleLHS=RuleList[x].ruleLHS;
           tempcle.ruleRHSpreDot=dummy;
        ...
        else if (RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS.find(dummy) == 0)//The non-terminal we are looking for is NOT directly given by a terminal in the RuleList, but is the first non-terminal on RHS
           {
           CykListElement tempcle;
           tempcle.ruleLHS=RuleList[x].ruleLHS;
           tempcle.ruleRHSpreDot=dummy;
...
for (int k=1;k<=maxk;k++)
    ...
    for (dwnitr=0;dwnitr < mat[DwnListPos].DownList.size();dwnitr++)//Run through the entire DownList of the cell at DwnListPos
        ...
        for (upitr=0;upitr < mat[UprListPos].UpList.size();upitr++)//Run through the entire UpList of the cell at UprListPos
            ...
            for (int x=0;x < RuleList[0].RuleCount;x++)//Run through each rule
                {
                if (RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS == dummy)//If the non-terminal set we are looking for is directly given by a terminal in the RuleList
                   {
                   CykListElement tempcle;
                   tempcle.ruleLHS=RuleList[x].ruleLHS;
                   tempcle.ruleRHSpreDot=dummy;
        	   ...
        	   if (!found)//If the CykList Element is not already prsent
                ...
                else if (RuleList[x].RuleType == _NONTERM_RULE && RuleList[x].ruleRHS.find(dummy) == 0)//The non-terminal set we are looking for is NOT directly given by a terminal in the RuleList, but is the first non-terminal set on RHS
                   {
                   CykListElement tempcle;
                   tempcle.ruleLHS=RuleList[x].ruleLHS;
                   tempcle.ruleRHSpreDot=dummy;
...
if (temp.ruleLHS.length()>0)
   {
   oss<<temp.ruleLHS.substr(1,temp.ruleLHS.length()-2);//This is to remove the leading and the trailing '*' (stars)
   oss<<" --> ";
...
while (!iss.eof() && !LoopOccured)
      ...
      if (findpos != string::npos)
         ...
         qasym->r=onlynode.substr(findpos+5);
      ...
      if (findpos != string::npos)
         ...
         qsym->r=onlynode.substr(findpos+4);
...
//A sample output can be seen in MatrixRef.txt (Grammar file used: smade02.txt, sequence 'agggcau')
/*
int x,y,abspos,itr;
