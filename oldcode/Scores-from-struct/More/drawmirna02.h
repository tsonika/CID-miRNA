...
struct miRNAstats
       {
       float n_stems,n_asyms,n_syms,n_bases,n_base_pairs,n_loop_bases;
...
miRNAstats getstats(string);//For Matrix
...
miRNAstats getstats(string inp)
{
...
//(isalpha(mat[1][c]) && isalpha(mat[3][c])) || isalpha(mat[2][c])
          if (isalpha(mat[1][c]) && isalpha(mat[3][c]))
             ...
             tempstats.n_bases+=2;//Since each Second last LOOP point has two bases, increase base count by 2
             temploopbases+=2;//Since this Second last LOOP point has two bases
             }
          else if (isalpha(mat[2][c]))
             {//Last position
             ++(tempstats.n_bases);//Since each Last LOOP point has one base, increase base count by 1
